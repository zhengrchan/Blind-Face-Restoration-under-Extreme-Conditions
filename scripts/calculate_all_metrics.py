...
if __name__ == '__main__':
    ...
    # opt['type'] = 'Celeb_Metrics_Dr2_Dataset'
    ...
    lpips = lpips_metric(lpips_vgg, data_loader, device)
    print(f'-> calculate lpips={lpips:.6f}, use {(time.time() - start)/60.:.2f}min')
    del lpips_vgg
    ...
    for img_path in img_list:
        ...
        psnr_score = calculate_psnr(img, gt_img, crop_border)
        psnr_all += psnr_score

        ssim_score = calculate_ssim(img, gt_img, crop_border)
        ssim_all += ssim_score

    niqe = niqe_all / len(img_list)
    psnr = psnr_all / len(img_list)
    ssim = ssim_all / len(img_list)
    ...
    print(f'| fid: {fid:.6f} | lpips: {lpips:.6f} | niqe: {niqe:.6f} | psnr: {psnr:.6f} | ssim: {ssim:.6f} |')
